...
% CORRECTED 2026-09-02 against the recovered BigQuery SQL. The previous text
% claimed an age >= 18 filter that was never applied, and reported the modeling
% requirement (11 SAS observations) as though it were the extraction filter
% (15 SAS observations and 5 propofol events). See instructions.txt Section 9.
Our cohort was extracted from MIMIC-IV v3.1 on 12 April 2026. Inclusion required at least fifteen Sedation-Agitation Scale assessments (itemid 223753, values restricted to the valid range of 1 to 7) and at least five propofol infusion events with a positive rate (itemid 222168) during the ICU stay. We applied no explicit age filter; MIMIC-IV contains only adult patients by construction, and the resulting cohort spans ages 18 to 93. Patient age was computed using MIMIC's anchor-year de-identification adjustment rather than read directly. Weight and height were drawn from the MIMIC-derived first-day measurement tables by outer join and were therefore missing for 1.2\% and 26.7\% of patients respectively, for whom we substituted 75\,kg and 170\,cm.

Separately from these extraction criteria, our temporal modeling requires at least eleven assessments per patient, since it uses the past ten observations to predict the next one. This modeling requirement is weaker than the extraction filter and never binds. The complete cohort selection logic is implemented in the SQL queries provided in our project repository.
